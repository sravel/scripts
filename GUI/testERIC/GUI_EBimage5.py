...
class EBimage( formEBimage, baseEBimage ):
	...
	def initialiseVariables(self):
		...
		self.dicoObjectOpenLineEditAnalysis = {
						"leafMinSize": self.ui.leafSizeLineEdit,
						"leafBorderSize": self.ui.leafBorderSizeLineEdit,
						"symptomMinSize": self.ui.lesionSizeLineEdit,
						"symptomBorderSize": self.ui.lesionBorderLineEdit
					 }

		self.dicoParamsAnalysis = {
						"leafMinSize": 1000,
						"leafBorderSize": 3,
						"symptomMinSize": 10,
						"symptomBorderSize": 3
					 }
		self.symptomColorOut = self.symptomColorComboBox.currentText()

		self.dicoObjectOpenLineEditAnalysisFolder = {
																			"input": self.ui.analysisOpenLineEdit, 
																			"outputAnalysisFolder": self.ui.calibrationFileOpenLineEdit, 
																			"RData": self.ui.pathOutlineEdit
																				}
		self.dicoFoldersAnalysis = {
											"input": None, 
											"outputAnalysisFolder": None, 
											"RData": None
											}
	...
	def createWidgets(self):
		...
		# Analysis
		self.ui.loadAnalysisPushButton.clicked.connect(lambda: self.loadFolderAnalysis(inputCat = "input", methode="clicked"))
		self.ui.analysisOpenLineEdit.editingFinished.connect(lambda: self.loadFolderAnalysis(inputCat = "input", methode="write"))

		self.ui.loadCalibrationFilePushButton.clicked.connect(lambda: self.loadFolderAnalysis(inputCat = "RData", methode="clicked"))
		self.ui.calibrationFileOpenLineEdit.editingFinished.connect(lambda: self.loadFolderAnalysis(inputCat = "RData", methode="write"))

		self.ui.loadPathOutPushButton.clicked.connect(lambda: self.loadFolderAnalysis(inputCat = "outputAnalysisFolder", methode="clicked"))
		self.ui.pathOutlineEdit.editingFinished.connect(lambda: self.loadFolderAnalysis(inputCat = "outputAnalysisFolder", methode="write"))

		self.ui.leafSizeLineEdit.editingFinished.connect(self.actualizeParamsAnalysis)
		self.ui.leafBorderSizeLineEdit.editingFinished.connect(self.actualizeParamsAnalysis)
		self.ui.lesionSizeLineEdit.editingFinished.connect(self.actualizeParamsAnalysis)
		self.ui.lesionBorderLineEdit.editingFinished.connect(self.actualizeParamsAnalysis)

	def actualizeParamsAnalysis(self):
		"""Actualize Parameters for analysis"""
		for key,  value in self.dicoParamsAnalysis.items():
			objectEdit = self.dicoObjectOpenLineEditAnalysis[key]
			currentValue = str(objectEdit.text())
			if currentValue != "":
				try:
					int(currentValue)
					self.dicoParamsAnalysis[key] = currentValue
				except Exception as e:
					self.displayError(error = "Please enter an integer value for parameters {} ({})!!!!".format(key,  e))
					objectEdit.setText(self.dicoParamsAnalysis[key])
			else:
				objectEdit.setText(self.dicoParamsAnalysis[key])

	def resetAnalysis(self):
		"""To reset if delete or change value"""
		for inputCat in self.dicoParamsAnalysis.keys():
			self.dicoParamsAnalysis[inputCat] = None
			self.dicoObjectOpenLineEditAnalysis[inputCat].setText(self.dicoParamsAnalysis[key])
	...
	def actualizeSymptomColor(self):
		"""Actualize outputAnalyseFolderut color analyse"""
		self.symptomColorOut = self.symptomColorComboBox.currentText()
	...
	def loadFolderAnalysis(self,inputCat = None, methode = None):
		"""Méthode qui permet de charger un fichier et afficher dans le plainText"""
		directoryToOpen = os.getcwd()
		if methode == "clicked":
			if inputCat in ["input",  "outputAnalysisFolder"]:
				pathdir = QFileDialog.getExistingDirectory(self, caption="Load the directory "+inputCat, directory=directoryToOpen)
			elif inputCat in ["RData"]:
				options = QFileDialog.Options()
				options = QFileDialog.DontUseNativeDialog
				pathdir, _ = QFileDialog.getOpenFileName(self, "Load the RData file ", directoryToOpen, "RData file (*.RData *.rdata);;All Files (*)" , options=options)
				print(inputCat)
			self.dicoObjectOpenLineEditAnalysisFolder[inputCat].setText(pathdir)
		elif methode == "write":
			pathdir = str(self.dicoObjectOpenLineEditAnalysisFolder[inputCat].text())
		if pathdir != "" and os.path.exists(pathdir):
			self.dicoFoldersAnalysis[inputCat] = pathdir
		else:
			self.dicoFoldersAnalysis[inputCat] = None
			self.dicoObjectOpenLineEditAnalysisFolder[inputCat].setText("")
			self.displayError(error = "\"%s\" is not a valid Path \n" % (pathdir))
