...
W <- df2genind(X=mvdata[,c(2:**MAKERS**)],pop=vect,missing="mean", ploidy=1,ncode=**NCODE**)

library(ape)
library(genetics)
library(spdep)

pca1 <- dudi.pca(W$tab, cent = TRUE, scale = FALSE, scannf = FALSE, nf = 3) #calcul de la PCA
...
s.class(pca1$li, W$pop, lab = W$pop.names, sub = "PCA 1-2", csub = 2) #graphique de la PCA
